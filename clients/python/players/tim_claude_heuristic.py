"""
tim_claude_heuristic — heuristic Hearts AI for tournament-style competition.

A separate lineage from claude_player. Where claude_player is a single
danger-scoring engine, this player layers six independent heuristics that
each address a distinct phase of the game:
...
Intended as a stronger panel opponent than random_player/rob_player and
stylistically distinct from claude_player.
"""
...
class TimClaudeHeuristic(Player):
    player_tag = "tim_claude_heuristic"
...
if __name__ == "__main__":
    ...
    with ManagedConnection() as conn:
        games = RunMultipleGames(
            conn, GameType.ANY,
            [TimClaudeHeuristic, RandomPlayer, RandomPlayer, RandomPlayer],
            num_games=20,
        )
        wins = sum(1 for g in games if g.winner.player_tag == "tim_claude_heuristic")
        print(f"TimClaudeHeuristic vs 3x Random: {wins}/20 wins")
