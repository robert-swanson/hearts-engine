...
class TimMCTSv1Augment(TimClaudePlayer):
    ...
    def _mcts_or_heuristic(
        ...
        # Always compute heuristic's choice first — it's our fallback and
        # also the move we'll defend in the paired comparison.
        ...
        # Candidate pruning: only evaluate top-K candidates by heuristic
        # preference (rank-low first, plus the heuristic's own pick).
        candidates = self._select_candidates(legal_moves, heuristic_move)
        if len(candidates) == 1:
            return candidates[0]

        # Paired-sample MCTS: same `opp_hands` for every candidate per
        # iteration, so we compare Δ per sample (CRN — much lower
        # comparison variance than independent samples).
        deadline = time.perf_counter() + self.search_time_budget
        # samples[candidate] -> list of scores under each sample
        ...
        if min(len(v) for v in samples.values()) < self.min_samples_to_override:
            return heuristic_move

        # Find MCTS-favored candidate (lowest mean score).
        def mean(card: Card) -> float:
            return sum(samples[card]) / len(samples[card])

        mcts_pick = min(candidates, key=mean)
        if mcts_pick == heuristic_move:
            return heuristic_move

        # Paired comparison: does MCTS pick beat heuristic by enough?
        if not self._mcts_significantly_better(samples[mcts_pick], samples[heuristic_move]):
            return heuristic_move
        return mcts_pick

    # Tunables for augment mode
    min_samples_to_override: int = 30
    # Effect-size threshold (mean delta in points/round); below this we
    # call it noise. 1.0 = MCTS pick must save at least 1 raw point.
    override_effect_threshold: float = 1.0
    # Top-K candidates to evaluate (by heuristic ordering).
    candidate_top_k: int = 3

    def _select_candidates(
        self, legal_moves: List[Card], heuristic_move: Card
    ) -> List[Card]:
        """Top-K candidates by heuristic preference. We don't have a
        cheap per-move heuristic score, so use a proxy: prefer low-rank
        legal moves (ducking style) plus the heuristic's actual pick.
        """
        if len(legal_moves) <= self.candidate_top_k:
            return list(legal_moves)
        sorted_by_rank = SortCardsByRank(legal_moves)
        picks = list(sorted_by_rank[: self.candidate_top_k])
        if heuristic_move not in picks:
            picks[-1] = heuristic_move  # ensure heuristic move is in set
        return picks

    def _mcts_significantly_better(
        self, mcts_samples: List[float], heur_samples: List[float]
    ) -> bool:
        """Paired one-sided check: does MCTS pick beat heuristic enough?
        Uses delta-of-paired-samples for the same opp_hand draws.
        """
        n = min(len(mcts_samples), len(heur_samples))
        if n < self.min_samples_to_override:
            return False
        deltas = [mcts_samples[i] - heur_samples[i] for i in range(n)]
        mean_delta = sum(deltas) / n
        if mean_delta >= -self.override_effect_threshold:
            return False  # not meaningfully better (or even worse)
        # Sample-variance t-test (one-sided, p<0.05 ≈ |t|>1.65).
        var = sum((d - mean_delta) ** 2 for d in deltas) / max(1, n - 1)
        se = (var / n) ** 0.5
        if se == 0:
            return True  # zero variance, mean already crossed threshold
        t = mean_delta / se
        return t < -1.65
